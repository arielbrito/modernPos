...
import { Plus, PencilLine, Trash2, Eye, Search, X, Loader2, StoreIcon } from "lucide-react";
...
export default function StoreIndex({ tiendas, filters }: Props) {
    ...
    React.useEffect(() => {
        if (debouncedSearch !== filters.q) {
            setIsSearching(true);
            router.get((stores.index()), { q: debouncedSearch || undefined }, {
                preserveState: true,
                preserveScroll: true,
                        ...
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div className="flex gap-2 w-full sm:w-auto">
                                ...
                                <Button onClick={handleCreate} className="gap-2">
                                    ...
                                    <span className="hidden sm:inline">Nueva Tienda o Sucursal</span>
                                </Button>
                            </div>
                        </div>
                    </CardHeader>
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {tiendas?.data.map((s) => (
                                        <TableRow key={s.id} className="hover:bg-muted/50">
                                            ...
                                            <TableCell>
                                                ...
                                                <div className="text-xs text-muted-foreground">{s.address || "Sin email"}</div>
                                            </TableCell>
                                            ...
                                            <TableCell className="text-right">
                                                <TooltipProvider delayDuration={100}>
                                                    <div className="flex justify-end gap-2">
                                                        <Tooltip><TooltipTrigger asChild><Link href={(stores.show.url({ store: Number(s.id) }))}><Button size="icon" variant="outline"><Eye className="h-4 w-4" /></Button></Link></TooltipTrigger><TooltipContent>Ver Detalles</TooltipContent></Tooltip>
                                    ...
                                    {tiendas?.data.length === 0 && (
                                        <TableRow><TableCell colSpan={5} className="h-24 text-center text-muted-foreground">No se encontraron tiendas.</TableCell></TableRow>
                                    )}
                                </TableBody>
                            </Table>
                        </div>
